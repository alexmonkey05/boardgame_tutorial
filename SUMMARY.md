...
- `main.py`가 FastAPI 앱을 제공하고 `storage.py`가 SQLite와 PostgreSQL 런타임을 선택합니다.
...
- 데이터 출처, 데이터·문구 라이선스, 검토일과 검토자를 함께 관리합니다.
- CSV 미리보기·오류 행·diff 확인, 전체 검증 적용 또는 유효 행 적용과 CSV 내보내기를 지원합니다.
- 중복 이름, 별칭 충돌, 관계 오류와 이미지 라이선스 경고를 품질 탭에서 확인하고 충돌 별칭·관계를 바로 제거할 수 있습니다.
...
- Python 컴파일 검사 통과
- 자동 테스트 `.venv\Scripts\python.exe -m pytest -q` 기준 `31 passed, 1 skipped`, Starlette TestClient/httpx deprecation 경고 1개
- Wikidata CC0 식별 메타데이터와 프로젝트 작성 문구를 사용한 51행 CSV가 전체 미리보기에서 오류 0개로 통과
- 초기 7개 데이터 품질 오류 0개·경고 0개·점수 100 확인
- 실제 PostgreSQL API 계약 테스트는 `TEST_POSTGRES_URL` 부재로 skip되며 시험 DB 승인 후 실행 필요
- 관리자 인증 누락·오류 `401` 검증
...
- 51행 운영 CSV 적용도 승인 전에는 수행하지 않았습니다.
